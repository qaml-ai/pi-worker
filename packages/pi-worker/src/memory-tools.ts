/**
 * In-memory filesystem tools for pi-mono's agent.
 *
 * Same interface as R2 tools but backed by a Map<string, string>.
 * Fast, strongly consistent, zero cost. Files don't survive Worker restarts.
 *
 * Uses the same path sanitization and edit algorithm as the R2 tools.
 */
...
export interface MemoryToolOptions {
	/** Key prefix. The agent never sees this prefix. */
	prefix?: string;
}
...
export function createMemoryReadTool(files: Map<string, string>, options?: MemoryToolOptions) {
	return {
		name: "read" as const,
		label: "read",
		...
		execute: async (_id: string, { path, offset, limit }: Static<typeof readSchema>) => {
			const content = files.get(toKey(path, options?.prefix));
...
export function createMemoryWriteTool(files: Map<string, string>, options?: MemoryToolOptions) {
	return {
		name: "write" as const,
		label: "write",
		description: "Write content to a file. Creates the file if it doesn't exist, overwrites if it does. You can create files in subdirectories (e.g. src/handlers/users.ts).",
		parameters: writeSchema,
		execute: async (_id: string, { path, content }: Static<typeof writeSchema>) => {
			files.set(toKey(path, options?.prefix), content);
...
// ---------------------------------------------------------------------------
// edit — same fuzzy match algorithm as R2 tools
// ---------------------------------------------------------------------------
...
export function createMemoryEditTool(files: Map<string, string>, options?: MemoryToolOptions) {
	return {
		name: "edit" as const,
		label: "edit",
		...
		execute: async (_id: string, { path, oldText, newText }: Static<typeof editSchema>) => {
			...
			files.set(key, bom + restoreLineEndings(result, ending));
...
export function createMemoryLsTool(files: Map<string, string>, options?: MemoryToolOptions) {
	return {
		name: "ls" as const,
		label: "ls",
		...
		execute: async (_id: string, { path }: Static<typeof lsSchema>) => {
			const basePrefix = options?.prefix ? `${options.prefix.replace(/\/+$/, "")}/` : "";
			const dirPath = (path || "").replace(/^\/+/, "").replace(/\/+$/, "").replace(/\/\/+/g, "/");
			const fullPrefix = dirPath ? `${basePrefix}${dirPath}/` : basePrefix;
			...
			for (const key of files.keys()) {
				...
				if (slashIdx === -1) {
					...
					entries.add(rest.substring(0, slashIdx + 1)); // directory/
				}
			}
...
/**
 * Create all four in-memory file tools at once.
 */
export function createMemoryTools(files: Map<string, string>, options?: MemoryToolOptions) {
	return [
		createMemoryReadTool(files, options),
		createMemoryWriteTool(files, options),
		createMemoryEditTool(files, options),
		createMemoryLsTool(files, options),
	];
}
